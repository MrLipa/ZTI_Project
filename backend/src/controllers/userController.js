...
const getUser = async (req, res) => {
  try {
    ...
    const userQuery = await pool.query(
      "SELECT * FROM zti_project.user WHERE user_id=$1",
      [user_id]
    );
  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          Message:
            "First Name, Last Name, Email, Password, Phone, Address, Image, Messages and Flights Ids are required.",
        });
    }
...
const deleteUser = (req, res) => {
  try {
    ...
    pool.query(
      "DELETE FROM zti_project.user WHERE user_id=$1",
      [user_id],
      (error, results) => {
        if (error) {
          res.status(500).json({ Error: error });
...
const getUserFlightsHistory = async (req, res) => {
  try {
    ...
    const userQuery = await pool.query(
      "SELECT flightIds FROM zti_project.user WHERE user_id=$1",
      [user_id]
    );
...
const makeReservation = async (req, res) => {
  try {
    ...
    pool.query(
      "UPDATE zti_project.user SET flightIds = array_append(flightIds, $1) WHERE user_id = $2",
      [flightId, user_id]
    );
    ...
    await axios.put(
      ...
      { flightId, freeSeats: flights.data[0].freeSeats - 1 },
      { headers: { Authorization: `Bearer ${token}` } }
    );
...
const cancelReservation = async (req, res) => {
  try {
    ...
    await pool.query(
      "UPDATE zti_project.user SET flightIds = $1 WHERE user_id = $2",
      [flightIds, user_id]
    );
